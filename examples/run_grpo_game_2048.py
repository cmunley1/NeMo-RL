...
from nemo_rl.environments.games.game_2048 import (
    Game2048Config,
    Game2048Env,
    Game2048GameLogic,
    Game2048Metadata,
...
def generate_2048_datum(
    ...
    """Generates a single 2048 game datum (prompt and metadata)."""

    def generate_random_config(max_config: Dict[str, Any]) -> Dict[str, Any]:
        """Generate a random config for the 2048 game."""
        return {
            "grid_size": max_config.get("grid_size", 4),
            "initial_tiles": max_config.get("initial_tiles", 2),
        }

    game_config = generate_random_config(game_config)
    initial_game_state = Game2048GameLogic.generate(game_config)
    initial_render = Game2048GameLogic.render(initial_game_state)
    welcome_message = Game2048GameLogic.init(initial_game_state)
    
    prompt_instructions = (
        f"{welcome_message}\n\n"
        f"Current Game State:\n{initial_render}\n\n"
        f"Your goal is to combine tiles to reach 2048 or the highest score possible.\n"
        f"Valid actions: 'up', 'down', 'left', 'right' to swipe in that direction.\n"
...
class Iterable2048Dataset(IterableDataset):
    """An IterableDataset that generates 2048 game data indefinitely."""
    ...
    def __iter__(self) -> Iterator[DatumSpec]:
        print("Starting Iterable2048Dataset (indefinite generation).")
        # Use itertools.count for an infinite index generator
        for i in itertools.count():
            yield generate_2048_datum(
...
def setup_2048_data(
    ...
    """Sets up the iterable data generator and env map for the 2048 game task."""
    print("Setting up 2048 game iterable data and environment...")
    env_config = env_cfg[task_name]

    print(f"Instantiating environment for task '{task_name}'...")
    env = Game2048Env.options(num_gpus=0).remote(cfg=dict(env_config["cfg"]))
    task_to_env = {task_name: env}
    print(f"Environment '{task_name}' created.")

    print("Creating 2048 game dataset...")
    training_dataset = Iterable2048Dataset(
        tokenizer=tokenizer,
    ...
    print("2048 game dataset created.")

    validation_dataset = Iterable2048Dataset(
        tokenizer=tokenizer,
        game_config=dict(env_config["cfg"]["game_config"]),
        max_moves=env_config["cfg"]["max_moves"],
        task_name=task_name,
...
def main():
    ...
    if not args.config:
        args.config = os.path.join(
            os.path.dirname(__file__), "configs", "grpo_game_2048.yaml"
        )
    ...
    dataset, val_dataset, task_to_env, val_task_to_env = setup_2048_data(
        tokenizer=tokenizer,
        env_cfg=config["env"],
        task_name="game_2048",
        length=ds_length,
